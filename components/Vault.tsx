...
import { VaultDates } from './VaultDates';
...
function VaultNotFound() {
    return (
        <div className="text-center py-12">
            <p className="text-gray-600 dark:text-white text-2xl">Vault not found</p>
        </div>
    )
}

function VaultDetail({ vault, address }: { vault: StakVault, address: string }) {
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{utilizationRate.toFixed(2)}%</p>
                ...
                <div className="mb-8 mt-20">
                    <div className="mb-4">
                        ...
                        <VaultDates start={vault.vestingStart} end={vault.vestingEnd} />
                    </div>
                    <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
...
export function Vault({ vaultPromise, vaultAddress }: { vaultPromise: Promise<StakVault>, vaultAddress: string }) {
    const vault = use(vaultPromise);

    if (!vault) return <VaultNotFound />

    return <VaultDetail vault={vault} address={vaultAddress} />;
}
